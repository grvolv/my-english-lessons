...
const STORAGE_PREFIX = "englishStoryWorkspace:v3:The_Leopard:A1_A2";
let data;
let selectedIndex = 0;
let playingIndex = -1;
let storyMode = false;
...
function builderRecord(scene = data.scenes[builderIndex]) {
...
function completedBuilderSentenceCount() {
  return data.scenes.filter((scene) => (builderProgress.sentences[scene.sentence_id]?.completed_modes || []).length > 0).length;
}
...
function lexicalMap() {
  const map = new Map();
  data.scenes.forEach((scene) => scene.lexical_items.forEach((item) => map.set(item.item_id, item)));
  return map;
}
...
function totalDuration() {
  return data.scenes.reduce((sum, scene) => sum + scene.duration_seconds, 0);
}

function elapsedDuration(local = 0) {
  return data.scenes.slice(0, Math.max(playingIndex, 0)).reduce((sum, scene) => sum + scene.duration_seconds, 0) + local;
}
...
function playStoryFrom(index) {
  ...
  if (index >= data.scenes.length) {
    ...
    storyMode = false;
    playingIndex = -1;
    $("playerStatus").textContent = "Full 30-sentence audio complete";
    updatePlaybackProgress(totalDuration());
    trackTrainerEvent("full_audio_completed", { scene_count: data.scene_count });
    return;
  }
  storyMode = true;
  ...
  selectedIndex = index;
  renderSelection();
  const nextAudio = new Audio(data.scenes[index].english_audio.path);
  audio = nextAudio;
...
function selectSentence(index, alreadyStopped = false) {
  ...
  renderSelection();
  $("playerStatus").textContent = `Ready at Sentence ${index + 1}`;
  trackTrainerEvent("sentence_selected", { sentence_id: data.scenes[index].sentence_id, scene: index + 1 });
}
...
function continueFromLearning(index = learningSourceIndex) {
  ...
  if (index >= data.scenes.length - 1) {
    selectedIndex = data.scenes.length - 1;
    renderSelection();
    storyPlaybackState = "complete";
    $("playerStatus").textContent = "Full 30-sentence audio complete";
    updatePlaybackProgress(totalDuration());
    trackTrainerEvent("full_audio_completed", { scene_count: data.scene_count, source: "learning_continue" });
    return;
  }
...
function renderSentences() {
  ...
  data.scenes.forEach((scene, index) => {
    ...
    article.tabIndex = -1;
    article.dataset.scene = String(scene.scene);
    const number = document.createElement("p");
    number.className = "sentence-number";
    number.textContent = `Sentence ${scene.scene}`;
    ...
    continueButton.addEventListener("click", () => playStoryFrom(Math.min(index + 1, data.scenes.length)));
...
function openLexicalItem(item, focusElement = document.activeElement) {
  ...
  const index = data.scenes.findIndex((scene) => scene.sentence_id === item.sentence_id);
...
function updateVocabularyContextActions() {
  ...
  $("vocabularyContinue").textContent = sceneNumber === data.scenes.length ? "Finish Story" : `Continue with Sentence ${sceneNumber + 1}`;
}
...
function openVocabulary(index = selectedIndex, focusElement = document.activeElement) {
  ...
  trackTrainerEvent("vocabulary_opened", { item_count: vocabulary.size, source_sentence_id: data.scenes[index].sentence_id });
}
...
function renderDeepStudy(index) {
  ...
  $("deepTitle").textContent = `Study Sentence ${scene.scene}`;
  ...
  scene.deep_study_blocks.forEach((block) => {
    ...
    english.lang = "en";
    english.textContent = `${block.position}. ${block.english}`;
    const ukrainian = document.createElement("p");
    ukrainian.lang = "uk";
    ukrainian.textContent = block.ukrainian;
    ...
    uaAudio.addEventListener("click", () => playFile(block.ukrainian_audio, "phrase_audio_played", { sentence_id: scene.sentence_id, block_id: block.ukrainian_id, language: "uk-UA" }));
    const actions = document.createElement("div");
    actions.className = "dialog-actions";
    actions.append(enAudio, uaAudio);
...
function openDeepStudy(index) {
  ...
  trackTrainerEvent("deep_study_opened", { sentence_id: data.scenes[index].sentence_id, scene: index + 1 });
}
...
function renderBuilderSentenceMenu() {
  ...
  data.scenes.forEach((scene, index) => {
    ...
    button.textContent = `Sentence ${scene.scene}${completed.length ? ` · ${completed.length}/2` : ""}`;
...
function resetBuilderRound() {
  const scene = data.scenes[builderIndex];
...
function chooseBuilderSentence(index) {
  ...
  builderIndex = index;
  builderMode = null;
  $("builderTitle").textContent = `Build Sentence ${data.scenes[index].scene}`;
...
function openSentenceBuilder(index = 0, direct = false) {
  ...
  trackTrainerEvent("sentence_builder_opened", {
    sentence_id: data.scenes[index].sentence_id,
    scene: index + 1,
...
function moveBankSegmentToAnswer(index, targetIndex = builderAnswer.length) {
  ...
  renderBuilderSegments();
  trackTrainerEvent("builder_segment_selected", { sentence_id: data.scenes[builderIndex].sentence_id, segment_id: segment.id, action: "placed" });
}
...
function moveAnswerSegmentToBank(index) {
  ...
  renderBuilderSegments();
  trackTrainerEvent("builder_segment_selected", { sentence_id: data.scenes[builderIndex].sentence_id, segment_id: segment.id, action: "removed" });
}
...
function handleBuilderDrop(event, targetLocation) {
  ...
  if (transfer.location === "bank" && targetLocation === "answer") moveBankSegmentToAnswer(transfer.index, targetIndex);
    ...
    renderBuilderSegments();
    trackTrainerEvent("builder_segment_selected", { sentence_id: data.scenes[builderIndex].sentence_id, segment_id: segment.id, action: "moved" });
  }
}
...
function checkBuilderAnswer() {
  ...
  if (correct) {
    ...
    if (newlyCompletedMode) {
      ...
      if ([5, 10, 15, 20, 25, 30].includes(completed)) {
        trackTrainerEvent("sentence_milestone_reached", { completed_sentence_count: completed, scene_count: data.scene_count });
      }
      if (completed === data.scene_count) {
        trackTrainerEvent("workspace_completed", { content_revision: data.content_revision, scene_count: data.scene_count });
      }
    }
  } else {
...
function resetBuilderProgressWithConfirmation(confirmAction = window.confirm) {
  if (!confirmAction("Reset all Sentence Builder progress for Sentences 1–30?")) return false;
  builderProgress = { sentences: {} };
  try { localStorage.removeItem(builderStorageKey()); } catch (_) {}
  renderBuilderSentenceMenu();
  resetBuilderRound();
  trackTrainerEvent("builder_progress_reset", { scene_count: data.scene_count });
  return true;
}
...
async function start() {
  ...
  if (data.scene_count !== 30 || data.scenes.length !== 30 || data.supported_lexical_item_count !== 239) throw new Error("Full Workspace data contract failed");
  renderSentences();
  data.scenes.forEach((scene, index) => {
    const option = document.createElement("option");
    option.value = String(index);
    option.textContent = `Sentence ${scene.scene}`;
  ...
  updateVocabularyCount();
  $("storyTime").textContent = `0:00 / ${formatTime(totalDuration())}`;
  trackTrainerEvent("trainer_open", { content_revision: data.content_revision, scene_count: data.scene_count, target_language: data.target_language });
}
...
$("playStory").addEventListener("click", () => {
  if (storyMode && audio.src && audio.paused && playingIndex >= 0) {
    ...
    resumedAudio.play().then(() => {
      ...
      trackTrainerEvent("full_audio_started", { start_sentence_id: data.scenes[playingIndex].sentence_id, resumed: true });
      updatePlaybackProgress(audio.currentTime);
    }).catch(() => {});
  } else {
    const startIndex = selectedIndex;
    playStoryFrom(startIndex);
    trackTrainerEvent("full_audio_started", { start_sentence_id: data.scenes[startIndex].sentence_id, resumed: false });
  }
});
$("pauseStory").addEventListener("click", () => {
  ...
  updatePlaybackProgress(audio.currentTime);
  trackTrainerEvent("full_audio_paused", { sentence_id: data.scenes[playingIndex].sentence_id });
});
$("restartStory").addEventListener("click", () => {
  selectedIndex = 0;
  playStoryFrom(0);
  trackTrainerEvent("full_audio_started", { start_sentence_id: data.scenes[0].sentence_id, restarted: true });
});
...
$("deepSentencePicker").addEventListener("change", (event) => {
  ...
  learningSourceIndex = index;
  renderDeepStudy(index);
  trackTrainerEvent("sentence_selected", { sentence_id: data.scenes[index].sentence_id, scene: index + 1, source: "deep_study_picker" });
});
$("deepEnglishAudio").addEventListener("click", () => playFile(data.scenes[selectedIndex].english_audio));
$("deepUkrainianAudio").addEventListener("click", () => playFile(data.scenes[selectedIndex].ukrainian_audio));
...
async function runPrototypeSelfTest() {
  ...
  try {
    ...
    check(document.querySelectorAll(".sentence-card").length === 30, "thirty rendered sentences");
    check(document.querySelectorAll(".lexical-item").length === 239, "239 supported lexical items");

    const first = data.scenes[0].lexical_items[0];
    ...
    data.scenes.forEach((scene, index) => {
      renderDeepStudy(index);
      check($("deepBlocks").children.length === 8, `deep study scene ${index + 1}`);
      check($("deepEnglish").textContent === scene.english && $("deepUkrainian").textContent === scene.ukrainian, `deep study content ${index + 1}`);
    });
    selectSentence(4);
    check(selectedIndex === 4 && document.querySelectorAll(".sentence-card")[4].classList.contains("selected"), "unrestricted sentence selection");
    ...
    data.scenes.forEach((scene) => {
      ...
      scene.deep_study_blocks.forEach((block) => {
        ...
        sources.add(block.ukrainian_audio.path);
      });
    });
    const responses = await Promise.all([...sources].map((source) => fetch(source, { cache: "no-store" })));
    check(responses.length === 525 && responses.every((response) => response.ok), "all 525 audio assets load");
